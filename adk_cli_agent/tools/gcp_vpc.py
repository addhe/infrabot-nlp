...
def create_vpc_network(
    ...
    try:
        ...
        if HAS_GCP_TOOLS_FLAG:
            ...
            # Use the same module as the client for model classes (so patching works)
            network = network_client.Network() if hasattr(network_client, "Network") else compute_v1.Network()
            network.name = network_name
            network.description = description
            network.auto_create_subnetworks = (subnet_mode.lower() == "auto")

            routing_config = network_client.NetworkRoutingConfig() if hasattr(network_client, "NetworkRoutingConfig") else compute_v1.NetworkRoutingConfig()
            if routing_mode.lower() == "regional":
                routing_config.routing_mode = routing_config.RoutingMode.REGIONAL
            else:
                routing_config.routing_mode = routing_config.RoutingMode.GLOBAL
